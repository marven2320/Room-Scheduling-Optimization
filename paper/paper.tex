...
\author{
\IEEEauthorblockN{Marven E. Jabian\IEEEauthorrefmark{1}, Stefany Mae Caparida}
...
\begin{itemize}
\item A fully specified, ten-constraint hard-constraint model (Section III) and a six-term multi-objective fitness function (Section IV-C) for departmental room/faculty/class scheduling, including the exact weighting used in a deployed system.
...
\item[H10] \textbf{Fixed-schedule respect}: a session imported through the Fixed Schedule tab as already decided (e.g. carried over from the previous semester's finished timetable) is pre-occupied in every relevant occupancy grid \emph{before} any new task is placed, so no newly-scheduled task may double-book its room, its faculty member (if one is given), or its regular-student cohort (if one is given) -- functionally indistinguishable, from the decoder's point of view, from a task the current run already placed.
\end{itemize}

Subject to H1--H10, the system optimizes six \textbf{soft objectives}, described formally in Section IV-C, plus two further soft \emph{preferences} applied during candidate selection rather than scored in the fitness function itself (Section IV-B): (i) maximize the number of sessions successfully placed; (ii) maximize room-utilization density; (iii) keep laboratory sessions within a 7:30 AM--6:00 PM preferred window; (iv) minimize the number of sessions placed on Friday; (v) preserve a guaranteed break window for every faculty member; and (vi) minimize idle gaps in each faculty member's daily schedule. Objective (i) strictly dominates the rest: a schedule that places one more session is always preferred over any improvement to (ii)--(vi), reflecting that an unplaced class is a functional failure the department must resolve manually, while (ii)--(vi) are quality-of-life refinements. The two additional preferences -- favoring an administrator-designated \emph{Priority Room} for a subject when one is free, and favoring a faculty member who would stay within a soft teaching-load ceiling -- are described alongside the candidate-ranking procedure they modify in Section IV-B, since neither is expressed as a term of Eq.~(\ref{eq:fitness}).

\section{Proposed Methodology}

\subsection{Design Rationale: Indirect Encoding over Direct Assignment}
A GA for timetabling can encode a candidate solution \emph{directly} (each gene fixes a task's room, day, and start slot outright) or \emph{indirectly} (each gene only orders the tasks, and a separate decoder assigns room/day/slot deterministically). A direct encoding lets crossover and mutation act straightforwardly on the assignment itself, but almost every direct-encoded offspring produced by naive crossover violates at least one hard constraint (H1--H10), requiring a repair step or a heavily penalized fitness function to steer the search back toward feasibility -- and with ten interacting hard constraints, designing a repair procedure that is both fast and unbiased is itself a substantial undertaking. This system instead uses the indirect, permutation-based design of Cicirello's survey \cite{cicirello2023}: because \emph{every} permutation decodes, through the greedy procedure of Section IV-B, into a schedule that already satisfies H1--H10 by construction (infeasible placements are simply deferred to the unscheduled list rather than accepted), order crossover and swap mutation can recombine and perturb chromosomes freely without ever producing an invalid schedule, and the entire feasibility burden is concentrated in one well-tested decoder rather than distributed across every genetic operator. The trade-off is that the GA does not directly control \emph{where} a task lands, only the \emph{order} in which tasks compete for resources -- Section VI-A's finding that the heuristic-seeded baseline already matches the full GA's placement rate suggests that, for the problem scales tested, this indirection costs little in practice.

\subsection{Chromosome Encoding}
The system uses a \emph{permutation encoding}: a chromosome is an ordered list of scheduling tasks. Each subject in the roster is first expanded into $\text{blocks}(s)$ independent, separately-scheduled copies -- e.g. two parallel intake sections of the same course, each competing for its own room, day, time, and faculty -- where $\text{blocks}(s)$ is an administrator-set Number of Blocks specific to the degree program $s$ belongs to (with a Default value for any subject not linked to an uploaded program), so different programs' course loads can be split into different numbers of sections within the same optimization run. Every block is then expanded into one or more tasks by \texttt{buildTasks()}: a non-split lecture or single-session laboratory becomes one \emph{single} task per weekly session; a 3-unit lecture eligible for the 1.5-hour split becomes one \emph{paired} task representing both weekly halves together (so the decoder can enforce H9 atomically); and a laboratory subject split across rooms for capacity reasons produces multiple independent single tasks, each with its own instance key so that its sections do not block each other off the same day. This expansion means the chromosome length (and hence the size of the search space, $|S|!$ over sequential placement decisions) reflects the actual number of scheduling decisions to be made, not just the number of subjects.

\subsection{Decoding Procedure}
Given an ordering (a chromosome), the system decodes it deterministically into a concrete schedule via a single left-to-right greedy pass (\texttt{runTrial}), maintaining per-room, per-faculty, and per-cohort occupancy grids that are updated as each task is placed. Before the first task is considered, every entry in the Fixed Schedule (H10) is written into these same grids as though it had already been placed in this very trial, so nothing new can be assigned into a room, faculty member, or cohort's slot that a fixed session already occupies:

\begin{enumerate}
\item For task $s_i$ in order, enumerate every $(r, d, t)$ triple satisfying H1--H10 given the \emph{already-placed} tasks (including every Fixed Schedule entry) before it -- a room/day/faculty/cohort candidate search, restricted to rooms allowing the subject's type and rooms/days within their configured availability.
...
\item Otherwise, candidates are ranked by the soft-objective heuristics of Section IV-C and two further soft \emph{preferences}, in this fixed order: laboratory time-window compliance first for LAB tasks, then Friday-avoidance, then an administrator-designated \emph{Priority Room} for the subject if one of its candidates is free (a per-room, per-subject soft preference set in the Rooms tab -- e.g. keeping a specialized lab for its equipment-matched subjects -- that still falls back to any other valid room when none of its priority rooms are available), then room-load/adjacency packing, then minimal wasted capacity. One candidate is then chosen: the top-ranked candidate with probability 0.7, or a uniformly random candidate among the top three otherwise. This controlled randomness lets different chromosome orderings explore genuinely different regions of the schedule space instead of always collapsing onto the same greedy choice. Independently of room ranking, whenever more than one qualified faculty member $f \in Q_i$ is free for the chosen $(d,t)$, the system prefers one who would remain within a soft \emph{teaching-load ceiling} of 24 units after taking on $s_i$ -- computed as 24 minus that faculty member's declared Admin/Research Load Units, compared against units already credited to them earlier in this same trial -- falling back to any free qualified faculty member if none would stay under the ceiling, since exceeding it is discouraged but never disallowed. A subject's own contribution to that ceiling is a flat 6 units (undergraduate) or 4.5 units (graduate) per distinct subject a faculty member ends up teaching, except any Laboratory session of 3 hours or longer, which always counts as 2.55 units regardless of level.
\item The chosen slot is marked occupied in the relevant occupancy grids, and the loop continues to the next task.
\end{enumerate}

Because every task is checked against constraints H1--H10 \emph{before} being placed, the decoder produces a hard-constraint-respecting schedule by construction. A final safety-net pass (\texttt{validateAndSplitConflicts}) re-scans the completed assignment set for any residual room, faculty, or cohort overlap and pulls the offending sessions back out as unscheduled with an explicit conflict reason, so a decoding edge case never silently ships a double-booking. Neither the Priority Room preference nor the faculty teaching-load ceiling appears as a term of the fitness function in Section IV-C: both act purely as tie-breaking preferences inside candidate ranking, so they influence \emph{which} otherwise-equally-valid slot or faculty member is chosen but never change how many sessions get placed or how a schedule's fitness is scored.
...
The optimizer described above is embedded in a client-side web application (Fig. \ref{fig:pipeline}) with no server-side dependency for scheduling itself: Rooms, Subjects, and Faculty are managed through dedicated tabs (with CSV import/export), and a curriculum \emph{Prospectus} can be uploaded per degree program (CSV, or best-effort PDF parsing) and linked to a \emph{Target Semester}, which auto-populates that term's subjects and activates the regular-student cohort constraint (H6) for every affected program and year level simultaneously, with an independently configurable Number of Blocks per program (Section IV-A) controlling how many parallel sections each program's course load expands into. A room can additionally be marked LEC-only, LAB-only, or Both (H1), given a shared-usage cap for rooms split with other departments (H8), and assigned one or more Priority Subjects (Section IV-B); a faculty member's record carries the subjects they are qualified to teach plus a declared Admin/Research Load Units figure, from which the soft teaching-load ceiling of Section IV-B is derived; and a \emph{Fixed Schedule} tab accepts a CSV of already-decided sessions (H10) -- typically the finished, hand-adjusted output of a previous run -- re-importable directly from an optimized result's own export, so one semester's final schedule can seed the next semester's constraints with no reformatting. Pressing \emph{Optimize} runs the algorithm of Sections IV-A--IV-F entirely in the browser and renders the result as a Room Timetable, a per-faculty schedule, a per-cohort regular-student schedule, and a flat list view, each printable and CSV-exportable; any task the decoder could not place is surfaced as a specific Room, Faculty, or Regular-Student conflict with the diagnostic reason identified during decoding (Section IV-B), rather than a generic failure. All scheduling data persists in the browser's local storage, so the system requires no backend database. An optional lightweight local server adds only anonymized usage tracking for app-functionality testing (Section V-B) and does not alter scheduling behavior.
...
\node[box, below=of ga] (decode) {Greedy decoder\\ \texttt{runTrial()} (H1--H10)};
...
Table \ref{tab:results} summarizes the controlled benchmark, re-run against the current deployed source (Section V-A) after this system's constraint model grew from nine to ten hard constraints plus two candidate-ranking preferences (Sections III, IV-B). Across all four scales, the deployed decoder placed the large majority of sessions in a single greedy pass already, since hard-constraint checking (H1--H10) is identical for the genetic algorithm and the baseline -- both use the same \texttt{runTrial} decoder. The genetic algorithm's contribution is therefore concentrated in the soft-objective terms.
...
\toprule
Scenario & \multicolumn{2}{c}{Placement (\%)} & \multicolumn{2}{c}{Fitness} & Gens & Pop & Time (ms) & $\Delta$Fitness \\
& GA & Greedy & GA & Greedy & (GA) & & (GA) & (GA$-$Greedy) \\
\midrule
Small & 83.2$\pm$19.5 & 83.2 & 1,209,961 & 1,209,911 & 34.8$\pm$34.9 & 83 & 2009$\pm$1895 & +51 \\
Medium & 96.3$\pm$5.7 & 96.3 & 3,589,828 & 3,589,638 & 9.1$\pm$7.9 & 150 & 3637$\pm$2946 & +190 \\
Large & 99.5$\pm$0.8 & 99.5 & 5,899,654 & 5,899,426 & 6.7$\pm$2.2 & 150 & 7072$\pm$2023 & +228 \\
XLarge & 98.8$\pm$2.8 & 98.8 & 8,239,464 & 8,239,150 & 4.3$\pm$0.5 & 150 & 9262$\pm$741 & +314 \\
\bottomrule
\end{tabular}
\end{table*}

Table~\ref{tab:results} shows that, in every one of the 40 trials run (10 per scale across all four scales), the genetic algorithm placed \emph{exactly} the same number of sessions as the greedy-only baseline -- a perfect 0-session difference in every trial, not just an average tie. This is expected once the shared decoder is considered: both the GA's best individual and the baseline are decoded through the identical constraint-respecting greedy procedure (\texttt{runTrial}), and the heuristic (most-constrained-first) ordering used as the baseline is also seeded into the GA's initial population and protected by elitism -- so the GA can never place \emph{fewer} sessions than the baseline, and in this benchmark, evidently found no ordering that placed \emph{more}. What the genetic algorithm consistently achieves instead is a higher overall \emph{fitness} than the single-pass baseline in every trial across all four scales -- a mean improvement of 51 (Small), 190 (Medium), 228 (Large), and 314 (XLarge) -- driven entirely by the soft-objective terms of Eq.~(\ref{eq:fitness}): tighter room-utilization packing (lower gapScore and active-room-day count), fewer Friday sessions, fewer late laboratory sessions, and more faculty members retaining a protected break window. This is a meaningful but modest result: it shows the evolutionary search reliably refines schedule \emph{quality} beyond the heuristic-seeded greedy pass, but on these synthetic instances it did not need to (and was not observed to) unlock additional \emph{feasibility} that the heuristic alone could not already reach -- a distinction the placement-dominant fitness function in Eq.~(\ref{eq:fitness}) was explicitly designed to preserve. Problem instances where the heuristic ordering performs poorly at placement (e.g. tightly constrained rosters with little slack) would be expected to show the GA's placement advantage more clearly; evaluating that case is left to future work (Section VII). Neither the Priority Room preference nor the faculty teaching-load ceiling introduced in Sections III and IV-B is exercised by this synthetic generator (no room is marked as a subject's priority, and no faculty member is given a nonzero Admin/Research Load Units value), so both remain neutral no-ops here by construction; Section VI-C discusses this as a threat to validity.

Generation count falls, and wall-clock time broadly rises, with problem size largely as the adaptive parameter formulas of Eq.~(\ref{eq:complexity})--(4) predict: the Small scenario (population 83, a sub-6000\,ms nominal time budget) averaged 34.8$\pm$34.9 generations in 2009$\pm$1895\,ms, while Medium, Large, and XLarge all saturate the population cap at 150 (Eq.~3) and therefore share the same 6000\,ms nominal ceiling (Eq.~4). Of those three, only Large (7072$\pm$2023\,ms, $\sim$1.18$\times$ the ceiling) and XLarge (9262$\pm$741\,ms, $\sim$1.54$\times$) actually overshot it in this run; Medium finished comfortably within budget at 3637$\pm$2946\,ms despite sharing the same nominal ceiling. This is a direct, measured consequence of the once-per-generation termination check described in Section IV-F: whichever generation happens to be in progress when the deadline is reached is always finished before the loop can stop, and how expensive that one generation's decode-and-evaluate pass turns out to be varies trial-to-trial with the stochastic evolutionary search itself -- so the overshoot is a real but non-deterministic property of the check, not a fixed multiplier of problem size, and can appear, shrink, or vanish between otherwise-identical runs of the same scenario. For the synthetic scales tested here this remains well within interactive bounds (under 10 seconds even at XLarge in this run), but it identifies a concrete limitation of the current termination check -- Section VII discusses checking the time budget within, rather than only between, generations for substantially larger departmental rosters.

The fitness improvement is not just consistent in direction but statistically significant: a paired $t$-test on the 10 per-scenario (GA, baseline) fitness pairs rejects the null hypothesis of no difference at $p<0.001$ for every scenario (Small: $t(9){=}5.20$, $p{=}5.6e-04$; Medium: $t(9){=}6.08$, $p{=}1.8e-04$; Large: $t(9){=}8.00$, $p{=}2.2e-05$; XLarge: $t(9){=}6.23$, $p{=}1.5e-04$), with the strongest evidence at Large scale. Because the same random problem instance is decoded by both methods in each trial, this is a matched-pairs design that controls for instance-to-instance difficulty variation, isolating the genetic algorithm's own contribution.

\subsection{Convergence Behavior}
Fig. \ref{fig:convergence} plots the best-of-generation fitness for a representative Medium-scenario trial. The trial (Medium-scale synthetic instance, 150 individuals per generation) placed all sessions from generation 1 and used its remaining 15 generations to refine the soft-objective terms; fitness rose from 3,999,732 to 3,999,774 (+42) across three visible improvement steps before the run terminated on the adaptive time budget.
...
\begin{axis}[
  ...
  ymin=3999725, ymax=3999780,
  xtick={2,4,6,8,10,12,14,16},
  scaled y ticks=false, yticklabel style={/pgf/number format/.cd, fixed, 1000 sep={,}},
  grid=major, grid style={dashed, gray!30},
  mark size=1.4pt,
]
\addplot[thick, color=blue, mark=*] coordinates { (1,3999732) (2,3999745) (3,3999745) (4,3999745) (5,3999750) (6,3999750) (7,3999751) (8,3999774) (9,3999774) (10,3999774) (11,3999774) (12,3999774) (13,3999774) (14,3999774) (15,3999774) (16,3999774) };
\end{axis}
\end{tikzpicture}
\caption{Best-of-generation fitness over the run for a representative Medium-scenario trial. The large initial jump reflects the dominance of the placement term in Eq. (\ref{eq:fitness}); subsequent generations refine room-utilization and faculty-preference sub-objectives in a series of small step improvements before plateauing.}
\label{fig:convergence}
\end{figure}

\subsection{Field Usage-Log Records}
The deployed application's usage-tracking log recorded 18 optimizer runs during development-stage testing, spanning problem sizes from 1 to 40 subjects and up to 5 faculty. 17 of 18 runs (94\%) placed every session with zero unscheduled conflicts; the single exception was a deliberately over-constrained 3-subject test case with only one room. The largest real run recorded -- 40 subjects, 130 sessions, 5 rooms, 5 faculty -- placed all 130 sessions. These field records are informal (ad hoc developer testing rather than a designed experiment) but are broadly consistent with the controlled benchmark's Medium/Large scenarios and support the same qualitative conclusion: the deployed system reliably reaches a fully conflict-free schedule at realistic departmental scale.

One correction to how this log has previously been read: the \texttt{latency\_ms} field it records (3.8\,ms to 306.2\,ms, mean 28.9\,ms, across the 18 runs) is the round-trip time of the fire-and-forget \texttt{POST} that reports each event to the local tracking endpoint, measured from just before the request is sent to just after its response arrives -- it is \emph{not} the optimizer's own algorithmic wall-clock time, since no corresponding ``optimization started'' event was logged to difference against at the time these 18 runs were recorded. All 18 runs above therefore corroborate \emph{that} they complete and \emph{how completely} they place sessions, but not \emph{how long} the optimizer itself took on real departmental data. This gap is now closed in the deployed application -- every optimizer run's own wall-clock time is measured directly around the call and logged as a new \texttt{optimizerWallMs} field alongside the existing telemetry -- so future field records will speak to real-world compute time as well; none of the 18 runs analyzed here predate that change, so no such measurements yet exist to report.

\subsection{Threats to Validity}
Four limitations bound how far these preliminary results generalize. First, the synthetic benchmark's subject/room/faculty distributions are plausible approximations of DEE's actual course mix, not a sample of real historical scheduling data; the field usage-log records partially address this but reflect ad hoc developer testing, not a full semester's real roster. Second, the regular-student cohort constraint (H6) -- the most combinatorially demanding of the ten hard constraints -- was not exercised in the controlled benchmark and is left to future work (Section VII). Third, the Fixed Schedule constraint (H10) and the two soft candidate-ranking preferences added since the constraint model's original nine-constraint version -- the Priority Room preference and the faculty teaching-load ceiling (Section IV-B) -- are likewise absent from the synthetic generator (no pre-existing fixed sessions, priority-room links, or nonzero Admin/Research Load Units are synthesized), so this benchmark demonstrates that adding them left the previously-measured behavior unchanged when they are unused, but does not yet characterize how the search behaves when they are actively in effect; that evaluation is also left to future work. Fourth, all trials were executed on a single machine; absolute wall-clock numbers will vary with hardware, though the \emph{relative} GA-vs-baseline comparison and the qualitative scaling trend are expected to hold.

\section{Conclusion and Future Work}

This paper presented the optimization core of a deployed Room Scheduling Optimization System for a university department: a permutation-encoded genetic algorithm with a deterministic, hard-constraint-respecting greedy decoder now enforcing ten hard constraints (including, most recently, respecting a Fixed Schedule of already-decided sessions) plus two soft candidate-ranking preferences (a per-room Priority Subject preference and a per-faculty teaching-load ceiling), a six-term multi-objective fitness function dominated by session placement, tournament selection, order crossover, swap mutation, elitism, and complexity-adaptive parameter scaling requiring no manual per-semester tuning. Preliminary evaluation through a controlled synthetic benchmark -- comparing the full genetic algorithm against a single-pass greedy-only baseline decoded through the identical constraint solver, across four problem scales and 40 trials, re-run against the current deployed source to confirm the growth in constraints left prior behavior unchanged when the new ones are unused -- together with field usage-log records from development testing of the deployed application, shows that the evolutionary search consistently and reliably improves the greedy baseline's schedule \emph{quality} (fitness) through its soft-objective terms, while matching its session-\emph{placement} rate exactly in every trial tested; computation stayed within a few seconds at every scale, though we also identified and report a measurable, non-deterministic time-budget overshoot at the two largest scales traceable to the algorithm's once-per-generation termination check.

Future work includes: (1) evaluating the system against a real, multi-program DEE semester roster exercising the regular-student cohort constraint (H6) at full combinatorial scale; (2) constructing tightly-constrained problem instances (little room/faculty slack) where the most-constrained-first heuristic is expected to under-perform, to isolate and quantify the genetic algorithm's contribution to \emph{placement feasibility} rather than schedule quality alone -- the present benchmark's heuristic-seeded baseline matched the full GA's placement rate in all 40 trials, so this remains an open question rather than a settled negative result; (3) checking the adaptive time budget \emph{within} a generation's decode-and-evaluate loop, not only between generations, to bound the measured wall-clock overshoot observed at the Large and XLarge scales (Section VI-A); (4) a controlled ablation over individual fitness-term weights and genetic operator choices (e.g., partially-mapped or cycle crossover in place of OX) to quantify each design decision's marginal contribution; (5) benchmarking against alternative metaheuristics (simulated annealing, constraint programming) on the same problem instances; (6) a post-deployment user study measuring the time DEE staff save relative to manual scheduling, and gathering structured feedback to refine the fitness function's weighting of soft objectives; and (7) extending the synthetic generator to actually exercise the Fixed Schedule constraint, the Priority Room preference, and the faculty teaching-load ceiling -- all confirmed neutral no-ops in the present benchmark (Section VI-C) -- to characterize the search's behavior when they are active rather than merely absent, now that field telemetry also captures the optimizer's own wall-clock time (Section VI-B) to correlate against.
