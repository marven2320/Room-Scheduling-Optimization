...
- **Rooms** — weekly availability grids (30-minute slots, 7:30 AM–9:00 PM), custom per-room hours, an **Allowable Usage %** cap for rooms shared with other departments, CSV import/export.
...
- **Optimizer** — a genetic algorithm (tournament selection, order crossover, swap mutation) that auto-scales population/generations to problem complexity, with a live convergence progress modal. Enforces room/faculty/regular-student/shared-room-usage constraints; anything that can't be placed is separated into Room / Faculty / Regular-Student conflict groups with specific, actionable reasons.
