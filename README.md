...
- **Optimizer** — a genetic algorithm (tournament selection, order crossover, swap mutation) that auto-scales population/generations to problem complexity, with a live convergence progress modal. Enforces room/faculty/regular-student/shared-room-usage/room-type constraints and prefers each room's manually-set Priority Subjects; anything that can't be placed is separated into Room / Faculty / Regular-Student conflict groups with specific, actionable reasons.
