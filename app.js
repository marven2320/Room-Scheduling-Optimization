...
let state = {
  ...
  // Fixed Schedule (Fixed Schedule tab) — sessions imported as already-decided and locked in
  // place. The optimizer treats every entry here as a hard constraint: its room/day/time is
  // pre-occupied before any new task is placed, so nothing new can double-book it, and (when
  // Faculty / Program+Year Level are given) it also blocks faculty and regular-student cohort
  // conflicts the same way a newly-scheduled session would. See runTrial()'s seedFixedSchedule.
  fixedSchedule: [], // [{id, subjectName, subjectType, roomId, roomName, day, startSlot, durationSlots, startMin, endMin, facultyId, facultyName, cohortGroup, blockIndex}]
  ...
  try{
    ...
    if(raw){
      ...
      if(parsed && typeof parsed === "object"){
        ...
        state.fixedSchedule = Array.isArray(parsed.fixedSchedule) ? parsed.fixedSchedule : [];
        state.schedule = parsed.schedule || null;

        let migratedAny = false;
        state.rooms.forEach(r=>{ if(migrateLegacyAvailability(r)) migratedAny = true; });
        if(migratedAny){
          // Slot indices shifted, so any saved schedule's (or fixed schedule's) startSlot
          // values no longer point at the right times — clear the optimized result rather
          // than show a silently-wrong timetable (re-generated by clicking Optimize again),
          // and warn rather than silently keep a Fixed Schedule whose times may now be wrong.
          state.schedule = null;
          if(state.fixedSchedule.length>0){
            alert("Room availability data from an older version was just upgraded, which can shift time-slot numbering. Please re-check your Fixed Schedule tab's entries against your intended times, and re-import if anything looks off.");
          }
          saveState();
        }
      }
...
function deleteRoom(id){
  if(!confirm("Delete this room? Any Fixed Schedule entries using it will be removed too. This cannot be undone.")) return;
  state.rooms = state.rooms.filter(r=>r.id!==id);
  const fixedCountBefore = state.fixedSchedule.length;
  state.fixedSchedule = state.fixedSchedule.filter(fx=>fx.roomId!==id);
  saveState();
  renderRooms();
  if(state.fixedSchedule.length!==fixedCountBefore) renderFixedSchedule();
...
function deleteFaculty(id){
  ...
  // Any Fixed Schedule entry assigned to them keeps its room/day/time (still fully enforced)
  // but loses the faculty-conflict check, same as an "unassigned" faculty elsewhere.
  state.fixedSchedule.forEach(fx=>{ if(fx.facultyId===id) fx.facultyId = null; });
  saveState();
  renderFaculty();
  renderFixedSchedule();
...
function renderScheduleTab(){
  ...
  html += `<div class="panel no-print">
    <div class="sched-toolbar" style="margin-bottom:0;">
      ...
      <button class="btn btn-ghost" id="export-fixed-btn" style="align-self:flex-end;" title="Downloads the full schedule (every room, not just the current view) in the format the Fixed Schedule tab imports.">📌 Export for Fixed Schedule</button>
    </div>
  </div>`;
  ...
  document.getElementById("export-fixed-btn").addEventListener("click", exportScheduleForFixedCsv);
  ...
  for(let i=0;i<NUM_SLOTS;i++){
    ...
    DAYS.forEach(d=>{
      ...
      if(cell && cell.type==="start"){
        ...
        const fixedTag = a.fixed ? `<div style="margin-top:3px;"><span class="tag-faculty" title="Imported from the Fixed Schedule tab — locked in place, not chosen by the optimizer.">🔒 fixed</span></div>` : "";
        html += `<td class="slot-booked" rowspan="${a.durationSlots}" style="background:${a.color||'#7a7a7a'}">${escapeHtml(a.subjectName)}<div class="t">${fmtTime(a.startMin)}–${fmtTime(a.endMin)}</div>${pairTag}${labTag}${roomTag}${facTag}${cohortTag}${externalTag}${fixedTag}</td>`;
...
function renderListView(sched){
  ...
  sorted.forEach(a=>{
    html += `<tr>
      <td><span class="dot" style="background:${a.color||'#7a7a7a'}"></span>${escapeHtml(a.subjectName)}${a.paired?` <span class="tag-pair">⇄ ${a.pairLabel}</span>`:""}${a.labSection?` <span class="tag-lab">LAB · Section ${a.labSection}</span>`:""}${a.isCohort?` <span class="tag-lec" title="${escapeHtml(cohortGroupLabel(a.cohortGroup))}">🎓 ${escapeHtml(cohortGroupLabel(a.cohortGroup))}</span>`:""}${a.external?` <span class="tag-external">🏢 external</span>`:""}${a.fixed?` <span class="tag-faculty" title="Imported from the Fixed Schedule tab.">🔒 fixed</span>`:""}</td>
...
/* ---------------------------------------------------------------------
   FIXED SCHEDULE
   Sessions imported as already-decided and locked in place — see the seeding block at the
   top of runTrial() for how these become hard constraints on the optimizer. The CSV format
   here (FIXED_SCHEDULE_HEADER) is deliberately what "Export for Fixed Schedule" (Optimize
   Schedule tab results) produces too, so a completed optimize run round-trips straight back
   in as next semester's starting point with no reformatting needed.
--------------------------------------------------------------------- */
const FIXED_SCHEDULE_HEADER = ["Subject","Type","Room","Day","Start","End","Faculty","Program","Year Level","Block"];

// One shared row-shape for both the schedule-results export and the Fixed Schedule tab's own
// export, so the two stay byte-for-byte compatible with each other and with what import expects.
function assignmentToFixedRow(a){
  const [program, yearLabel] = a.cohortGroup ? a.cohortGroup.split("|") : ["", ""];
  return [
    a.subjectName || "",
    a.subjectType || (a.paired ? "LEC" : ""),
    a.roomName || "",
    DAY_FULL[a.day] || a.day,
    fmtTime(a.startMin),
    fmtTime(a.endMin),
    a.facultyName || "",
    program,
    yearLabel,
    a.cohortGroup ? String((a.blockIndex||0)+1) : ""
  ];
}

// "Export for Fixed Schedule" (Optimize Schedule tab results) — the FULL schedule regardless
// of the current View/filter, in the Fixed Schedule tab's own import format. External
// Assignment sessions have no real room (TBA), so there's nothing there for a Fixed entry to
// pin in place — they're left out, same as they're excluded from room-conflict checking
// elsewhere in the app.
function exportScheduleForFixedCsv(){
  const sched = state.schedule;
  if(!sched || sched.assignments.length===0){ alert("No scheduled sessions to export yet."); return; }
  const fixable = sched.assignments.filter(a=> a.roomId && a.roomId!==TBA_ROOM_ID);
  if(fixable.length===0){ alert("Every scheduled session is External Assignment (TBA/TBD), which has no real room to pin in place — nothing to export for a Fixed Schedule."); return; }
  const rows = [FIXED_SCHEDULE_HEADER];
  fixable.slice()
    .sort((a,b)=> DAYS.indexOf(a.day)-DAYS.indexOf(b.day) || a.startSlot-b.startSlot)
    .forEach(a=> rows.push(assignmentToFixedRow(a)));
  downloadCsv("schedule-fixed-export.csv", rows);
  trackEvent("exportScheduleForFixed", { extra:{ rows: rows.length-1 } });
}

function exportFixedScheduleCsv(){
  if(state.fixedSchedule.length===0){ alert("No fixed schedule entries to export yet."); return; }
  const rows = [FIXED_SCHEDULE_HEADER];
  state.fixedSchedule.slice()
    .sort((a,b)=> DAYS.indexOf(a.day)-DAYS.indexOf(b.day) || a.startSlot-b.startSlot)
    .forEach(fx=> rows.push(assignmentToFixedRow(fx)));
  downloadCsv("fixed-schedule.csv", rows);
  trackEvent("exportFixedScheduleCsv", { numFixed: state.fixedSchedule.length });
}

function downloadFixedScheduleTemplate(){
  downloadCsv("fixed-schedule-template.csv", [
    FIXED_SCHEDULE_HEADER,
    ["EEE101", "LEC", "Room 101", "Monday", "9:00 AM", "10:30 AM", "Engr. Dela Cruz", "BS Electrical Engineering", "First Year", "1"],
    ["EEE101", "LEC", "Room 101", "Wednesday", "9:00 AM", "10:30 AM", "Engr. Dela Cruz", "BS Electrical Engineering", "First Year", "1"],
    ["PE1", "LEC", "Gym", "Tuesday", "1:00 PM", "2:00 PM", "", "", "", ""]
  ]);
}

// Full day name ("Monday") -> the app's short code ("Mon") — the inverse of DAY_FULL. Also
// accepts an already-short code as-is, so a file built from this app's own exports (which
// already write full names) and a hand-edited one that kept the short form both work.
const FULL_NAME_TO_DAY = Object.fromEntries(DAYS.map(d=> [DAY_FULL[d].toLowerCase(), d]));
function dayFromFullName(name){
  const raw = String(name||"").trim();
  if(DAYS.includes(raw)) return raw;
  return FULL_NAME_TO_DAY[raw.toLowerCase()] || null;
}

// Room and Faculty are matched by name (case-insensitive), the same identity rule used
// elsewhere in the app (prospectus programs, faculty CSV import) — Room is required (a Fixed
// entry with no real room can't block anything, so unmatched rows are skipped and counted);
// Faculty is optional, and an unmatched name is kept for display only (no faculty-conflict
// blocking for that entry) rather than rejecting the whole row over it. Program + Year Level
// together are also optional and, when given, make this entry block regular-student cohort
// conflicts too — same "program|yearLabel" key buildCohortGroups() uses, so a Fixed entry
// naturally lines up with cohorts coming from the uploaded Prospectus without needing an
// actual Prospectus Course record to exist.
function importFixedScheduleFromRows(dataRows){
  let added = 0, skippedFields = 0, skippedRoom = 0, skippedTime = 0, duplicates = 0;
  const seen = new Set(state.fixedSchedule.map(fx=> [fx.roomId, fx.day, fx.startSlot].join("|")));
  dataRows.forEach(cols=>{
    const subjectName = (cols[0]||"").trim();
    const subjectType = (cols[1]||"").trim().toUpperCase()==="LAB" ? "LAB" : "LEC";
    const roomName = (cols[2]||"").trim();
    const dayRaw = (cols[3]||"").trim();
    const startRaw = (cols[4]||"").trim();
    const endRaw = (cols[5]||"").trim();
    const facultyName = (cols[6]||"").trim();
    const program = (cols[7]||"").trim();
    const yearLabel = (cols[8]||"").trim();
    const blockRaw = (cols[9]||"").trim();

    if(!subjectName || !roomName || !dayRaw || !startRaw || !endRaw){ skippedFields++; return; }

    const room = state.rooms.find(r=> r.name.trim().toLowerCase()===roomName.toLowerCase());
    if(!room){ skippedRoom++; return; }

    const day = dayFromFullName(dayRaw);
    const startMin = parseTimeLabel(startRaw), endMin = parseTimeLabel(endRaw);
    if(!day || startMin==null || endMin==null || endMin<=startMin){ skippedTime++; return; }
    const startSlot = SLOT_TIMES.indexOf(startMin);
    const durationSlots = Math.round((endMin-startMin)/SLOT_LEN);
    if(startSlot<0 || durationSlots<=0 || startSlot+durationSlots>NUM_SLOTS){ skippedTime++; return; }

    const dupKey = [room.id, day, startSlot].join("|");
    if(seen.has(dupKey)){ duplicates++; return; }
    seen.add(dupKey);

    const faculty = facultyName ? findFacultyByName(facultyName) : null;
    const cohortGroup = (program && yearLabel) ? `${program}|${yearLabel}` : null;
    const blockNum = parseInt(blockRaw, 10);
    const blockIndex = (cohortGroup && !isNaN(blockNum) && blockNum>=1) ? blockNum-1 : 0;

    state.fixedSchedule.push({
      id: genId("fixed"),
      subjectName, subjectType,
      color: "#7a7a7a", // muted gray, consistently distinct from the GA's per-subject color rotation
      roomId: room.id, roomName: room.name,
      day, startSlot, durationSlots,
      startMin: SLOT_TIMES[startSlot], endMin: SLOT_TIMES[startSlot]+durationSlots*SLOT_LEN,
      facultyId: faculty ? faculty.id : null,
      facultyName: faculty ? faculty.name : (facultyName || null),
      cohortGroup, blockIndex, isCohort: !!cohortGroup
    });
    added++;
  });
  saveState();
  renderFixedSchedule();
  trackEvent("importFixedScheduleCsv", { extra:{ added, skippedFields, skippedRoom, skippedTime, duplicates } });
  return { added, skippedFields, skippedRoom, skippedTime, duplicates };
}

function deleteFixedEntry(id){
  state.fixedSchedule = state.fixedSchedule.filter(fx=>fx.id!==id);
  saveState();
  renderFixedSchedule();
  trackEvent("deleteFixedEntry");
}
function clearFixedSchedule(){
  if(state.fixedSchedule.length===0) return;
  if(!confirm("Clear the entire Fixed Schedule? This cannot be undone.")) return;
  state.fixedSchedule = [];
  saveState();
  renderFixedSchedule();
  trackEvent("clearFixedSchedule");
}

function renderFixedSchedule(){
  const badge = document.getElementById("badge-fixed");
  if(badge) badge.textContent = state.fixedSchedule.length;
  const container = document.getElementById("fixed-schedule-list");
  if(!container) return;
  if(state.fixedSchedule.length===0){
    container.innerHTML = '<div class="empty">No fixed schedule entries yet. Import a CSV above, or optimize a schedule on the Optimize Schedule tab and use "📌 Export for Fixed Schedule" there.</div>';
    return;
  }
  const sorted = state.fixedSchedule.slice().sort((a,b)=> DAYS.indexOf(a.day)-DAYS.indexOf(b.day) || a.startSlot-b.startSlot);
  container.innerHTML = `<table class="list-table">
    <thead><tr><th>Subject</th><th>Room</th><th>Day</th><th>Time</th><th>Faculty</th><th>Cohort</th><th></th></tr></thead>
    <tbody>${sorted.map(fx=> `
      <tr>
        <td><span class="dot" style="background:#7a7a7a"></span>🔒 ${escapeHtml(fx.subjectName)}</td>
        <td>${escapeHtml(fx.roomName)}</td>
        <td>${DAY_FULL[fx.day]}</td>
        <td>${fmtTime(fx.startMin)} – ${fmtTime(fx.endMin)}</td>
        <td>${fx.facultyName ? escapeHtml(fx.facultyName) : `<span class="tag-faculty on-light">unassigned</span>`}</td>
        <td>${fx.cohortGroup ? escapeHtml(cohortGroupLabel(fx.cohortGroup)) + (fx.cohortGroup ? ` (Block ${(fx.blockIndex||0)+1})` : "") : ""}</td>
        <td><button class="btn btn-sm btn-danger" data-action="delete-fixed" data-id="${fx.id}">Delete</button></td>
      </tr>`).join("")}
    </tbody>
  </table>`;
}
...
/* --- Fixed Schedule tab wiring --- */
document.getElementById("fixed-export-btn").addEventListener("click", exportFixedScheduleCsv);
document.getElementById("fixed-template-btn").addEventListener("click", downloadFixedScheduleTemplate);
document.getElementById("fixed-import-btn").addEventListener("click", ()=> document.getElementById("fixed-import-file").click());
document.getElementById("fixed-import-file").addEventListener("change", (e)=>{
  const file = e.target.files[0];
  if(file) handleCsvImport(file, importFixedScheduleFromRows, (r)=>
    `Imported ${r.added} fixed session(s).`
    + (r.skippedRoom ? ` Skipped ${r.skippedRoom} row(s) whose Room didn't match an existing room (Rooms tab).` : "")
    + (r.skippedTime ? ` Skipped ${r.skippedTime} row(s) with an unreadable Day/Start/End.` : "")
    + (r.skippedFields ? ` Skipped ${r.skippedFields} row(s) missing Subject/Room/Day/Start/End.` : "")
    + (r.duplicates ? ` Skipped ${r.duplicates} duplicate(s) already in the Fixed Schedule.` : "")
  , "subject");
  e.target.value = "";
});
document.getElementById("fixed-clear-btn").addEventListener("click", clearFixedSchedule);
document.getElementById("fixed-schedule-list").addEventListener("click", (e)=>{
  const delBtn = e.target.closest('[data-action="delete-fixed"]');
  if(delBtn) deleteFixedEntry(delBtn.dataset.id);
});
